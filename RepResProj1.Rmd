...
```
The average is `r averagebyday` and the median is `r medianbyday`.
...
```
There are `r print(num_NA)` NA values in the raw data. One way to do this would be to use the average computed value from all days for any given interval. We can do this with the following code:
...
```
Which are now `r averagenona` and `r mediannona`.
